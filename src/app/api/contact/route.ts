...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          company,
          email,
          phone: phone ? `${countryCode || ""} ${phone}`.trim() : null,
          category: categories && categories.length > 0 ? categories.join(", ") : "General Inquiry",
    ...
    const safe = {
      ...
      phone: sanitize(phone ? `${countryCode || ""} ${phone}`.trim() : "Not provided"),
      category: sanitize(categories && categories.length > 0 ? categories.join(", ") : "Not specified"),
